...
export default function CreateCapsule() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult('Capsule created successfully! ID: ' + Math.random().toString(36).substr(2, 9))
    } catch (error) {
        ...
        {result ? (
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-8 space-y-6">
            ...
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="My Secret Document" />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Notes</label>
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500" rows={3} />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Approver Public Key</label>
              <input type="text" value={approverKey} onChange={(e) => setApproverKey(e.target.value)} className="w-full px-4 py-2 border rounded-lg font-mono text-sm" placeholder="ed25519:..." required />
            </div>
            ...
            <div className="border-t pt-6">
              ...
              <div className="flex items-center">
                ...
                <label htmlFor="useLocation" className="ml-2 text-sm text-gray-700">Require specific location</label>
              </div>
            </div>
