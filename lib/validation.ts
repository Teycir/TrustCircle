export function validatePublicKey(key: string): void {
  ...
  if (parts.length !== 2 || !['ed25519', 'x25519'].includes(parts[0])) {
    throw new Error('Invalid key format. Expected "ed25519:..." or "x25519:..."')
  }
...
export function sanitizeMetadata(metadata: any): any {
  const allowed = ['title', 'notes', 'version', 'created_at', 'unlock_policy', 'encrypted_cmk', 'metadata_sig', 'creator_pubkey', 'approver_pubkey', 'payload_cid']
  const sanitized: any = {}
